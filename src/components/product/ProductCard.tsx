...
const ProductCard: React.FC<ProductProps> = ({ product, onClick }) => {
  return (
    <div className="border border-gray-300 bg-white cursor-pointer hover:scale-105 transition-transform duration-300 rounded-2xl shadow-md w-full h-[320px] md:h-[380px] flex flex-col" onClick={onClick}>
      <img src={product.photo_url} alt={`image of ${product.title}`} className="w-full h-44 md:h-56 object-cover rounded-t-2xl mb-2" />
      <div className="p-3 flex flex-col flex-grow justify-between">
        <div>
          <h3 className="font-semibold text-sm md:text-base truncate">{product.title}</h3>
          <p className="font-bold text-primary-400 text-sm md:text-lg">Rp{formatPrice(product.price)}/hari</p>
        </div>
        <div>
          <p className="text-xs md:text-base truncate">Kecamatan, Nama Jalan</p>
